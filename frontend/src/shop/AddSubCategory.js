...
const AddSubCategory = () => {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            if (user?.profile?._id) {
                try {
                    const response = await fetch(`http://localhost:5510/grocery/getcategories/${user.profile._id}`);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (resData.success) {
                navigate('/shop', { state: { activeTab: 'subcategories' } });
            } else {
                    ...
                    {[
                        ...
                        <button
                            ...
                            onClick={() => {
                                ...
                                else if (item.id === 'addsubcategory') navigate('/shop/addsubcategory');
                                else {
